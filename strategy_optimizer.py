...
def find_best_strategy(segments, car: CarParams, total_laps: int, include_2stop: bool = True,
                        step: float = 20.0, verbose: bool = True):
    ...
    for plan in all_plans:
        res = simulate_race_strategy(segments, car, plan, total_laps, step=step)
...
if __name__ == "__main__":
    ...
    results = find_best_strategy(MONZA_SEGMENTS, car, TOTAL_LAPS, include_2stop=True, step=20.0)

    print(f"\nTop 10 strategies for Monza ({TOTAL_LAPS} laps, "
          f"pit loss = {PIT_STOP_LOSS_S}s):\n")
